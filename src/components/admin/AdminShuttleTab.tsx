...
export const AdminShuttleTab: React.FC<{
  ...
  const saveRoute = async () => {
    ...
    if (routeForm.selected_students.some(student => !student.pickup_spot_id)) return window.alert('모든 학생의 승차·하차 정류장을 해당 노선에서 지정해 주세요.');
...
const RouteEditor = ({ form, branchId, spots, schedules, setForm, saving, onClose, onSave }: {
  ...
  const addChild = (child: any) => {
    ...
    updateForm({ selected_students: [...form.selected_students, {
      ...
      class_schedule_id: schedule.id, direction: 'pickup', pickup_spot_id: '', custom_time: '',
    }] });
    setSearch('');
  };
